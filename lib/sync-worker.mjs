  ...
  for (const [slug, sheet] of Object.entries(CONFIG)) {
    ...
    try {
      ...
      for (const tab of tabsToProcess) {
        ...
        if (normTabName === "ads uploaded 2026" || normTabName === "old | ad tracker" || normTabName.includes("archive")) {
        ...
        for (let i = headerIdxFound + 1; i < dataToProcess.length; i++) {
          ...
          const getExactCol = (exactNames) => {
            ...
            const colIdx = headerRow.findIndex(h => names.includes(String(h).toLowerCase().trim()));
          ...
          if (tabType === "feedback") {
            ...
            creatives_folder = getExactCol(["image_url", "format_variants"]);

            if (!concept || !creatives_folder) {
              stats.invalid++;
              continue;
            }
          ...
          allRows.push({
            ...
            creativeSources: creatives_folder ? [{ type: 'drive', url: creatives_folder }] : [],
            routing: {
