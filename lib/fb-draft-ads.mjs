...
// Derive the page an account actually runs ads from (modal page_id across its
// recent ads). Self-calibrating, so QA works for every client without a
// hardcoded page per account. Returns "" if it cannot be determined.
export async function accountCanonicalPage(token, secret, acct) {
  try {
    const r = await graphGet(
      token,
      secret,
      `act_${acct}/ads?fields=creative{object_story_spec{page_id}}&limit=40`
    );
    const counts = {};
    ...
    let best = "", n = 0;
    for (const [pid, c] of Object.entries(counts)) if (c > n) { best = pid; n = c; }
    return best;
  } catch {
    return "";
  }
