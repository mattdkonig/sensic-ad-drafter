...
// hardcoded page per account. Returns { pageId: "", instagramActorId: "" } if it cannot be determined.
export async function accountCanonicalIdentities(token, secret, acct) {
  try {
    ...
    for (const ad of r.data || []) {
      ...
      const iid = ad?.creative?.object_story_spec?.instagram_actor_id || ad?.creative?.object_story_spec?.instagram_user_id;
      if (iid) iCounts[iid] = (iCounts[iid] || 0) + 1;
    }
    let bestP = "", nP = 0;
    for (const [pid, c] of Object.entries(pCounts)) if (c > nP) { bestP = pid; nP = c; }
    let bestI = "", nI = 0;
    for (const [iid, c] of Object.entries(iCounts)) if (c > nI) { bestI = iid; nI = c; }
    return { pageId: bestP, instagramActorId: bestI };
  } catch {
...
export async function createCleanDraft({ token, secret, accountId, adsetId, plan, imageHash, imageUrl, instagramActorId }) {
...
export async function createCleanVideoDraft({ token, secret, accountId, adsetId, plan, videoId, thumbnailUrl, instagramActorId }) {
  const acct = String(accountId || "").replace(/^act_/, "");
  if (!acct) throw new Error("accountId required");
  if (!adsetId) throw new Error("adsetId required");
  if (!plan?.page_id) throw new Error("plan.page_id required");
...
export async function createAssetCustomizationDraft({ token, secret, accountId, adsetId, plan, images, videos, instagramActorId }) {
  const acct = String(accountId || "").replace(/^act_/, "");
  if (!acct) throw new Error("accountId required");
  if (!adsetId) throw new Error("adsetId required");
  if (!plan?.page_id) throw new Error("plan.page_id required");
  ...
  for (const [format, group] of Object.entries(formatGroups)) {
    ...
    spec.publisher_platforms = ["facebook", "instagram", "messenger"];
