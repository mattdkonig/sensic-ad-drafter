...
const oldVideoCheck = `        const ready = await waitVideoReady({ ...fbArgs(env), videoId: v.id });
...
const newVideoCheck = `        const ready = await waitVideoReady({ ...fbArgs(env), videoId: v.id });
        if (ready === "error") { hasError = true; errorMsg = "Meta could not process a video"; break; }
        if (ready !== "ready") { 
          hasError = true; 
